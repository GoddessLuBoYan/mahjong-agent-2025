...
class FeatureAgent(MahjongGBAgent):
    ...
    tuibudao_list = "B1 B2 B3 B4 B5 B8 B9 T2 T4 T5 T6 T8 J3".split(" ")  # 注意，J1J2J3依次为中发白，白是J3不是J1
    lvyise_list = "T2 T3 T4 T6 T8 J2".split(" ")

    def get_dianshu_offset(self, tile):
        tlist = "F1 F2 F3 F4 J1 J2 J3".split(" ")
        if tile[0] in "FJ":
            return 9 + tlist.index(tile)  # 0-6 --> 9-15
        return int(tile[1]) - 1  # 0-8
    ...
    class OFFSET_OBS:
        ...
        DIANSHU = 60  # +16, 每张牌的点数1-9，东南西北中发白10-16
        LEIXING = 76  # +5，万条饼字将
        IS_YAOJIU = 81  # +1，是否为幺九牌
        IS_ZIPAI = 82  # +1，是否为字牌
    ...
    def __init__(self, seatWind):
        ...
        for tile in self.TILE_LIST:
            self.obs[self.OFFSET_OBS.DIANSHU + self.get_dianshu_offset(tile), self.OFFSET_TILE[tile]] = 1
            self.obs[self.OFFSET_OBS.LEIXING + self.get_leixing_offset(tile), self.OFFSET_TILE[tile]] = 1

        # 初始化幺九牌、字牌、推不倒牌、绿一色牌的信息
        for tile in self.yaojiu_list:
            self.obs[self.OFFSET_OBS.IS_YAOJIU, self.OFFSET_TILE[tile]] = 1
        for tile in self.zipai_list:
            self.obs[self.OFFSET_OBS.IS_ZIPAI, self.OFFSET_TILE[tile]] = 1
        for tile in self.tuibudao_list:
